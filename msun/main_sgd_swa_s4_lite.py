...
class MultiScaleResNet(lightning.LightningModule):
    ...
    def forward_random(self, x: torch.Tensor) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        # choose one index from the first k-1 subnets, then the last subnet
        picks = [random.randrange(len(self.subnets) - 1), -1]
        zs, ys = [], []
        for i in picks:
            r = random.choice(self.res_lists[i])
            z = self.subnets[i](F.interpolate(x, (r, r), mode='bilinear', align_corners=False))
            zs.append(z)
            ys.append(self.unified_net(F.interpolate(z, size=self.z_size,
                                                     mode='bilinear', align_corners=False)))
