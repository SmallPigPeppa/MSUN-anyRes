...
for model in "${models[@]}"; do
  ...
  python3 mixedres/main_sgd_swa_s1.py fit \
    ...
    --trainer.logger.project mixedres-anyres \
    --trainer.logger.name "mixedres-s1-$model" \
    --trainer.logger.log_model False \
    --trainer.logger.offline False \
    --trainer.gradient_clip_val 0.5 \
    --swa.swa_lrs 1e-2 \
    --swa.swa_epoch_start "$swa_epoch_start" \
    --model_checkpoint.dirpath "/mnt/bn/liuwenzhuo-hl-data/ckpt/msun/mixedres-s1/$model" \
